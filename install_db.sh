...
cp data/shs.sql /tmp/shs.sql

sudo sed -i "s/MY_SRID_WORLD/$srid_world/g" /tmp/shs.sql


# Create SHS schema into GeoNature database
echo "Creating 'SHS' schema... "
export PGPASSWORD=$user_pg_pass;psql -h $db_host -U $user_pg -d $db_name -f /tmp/shs.sql &>> var/log/install_shs.log
...
then
  ...
  cp data/shs_data.sql /tmp/shs_data.sql
  export PGPASSWORD=$user_pg_pass;psql -h $db_host -U $user_pg -d $db_name -f /tmp/shs_data.sql &>> var/log/install_shs_data.log
fi
