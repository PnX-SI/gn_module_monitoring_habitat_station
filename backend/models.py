...
class HabrefSHS(DB.Model):
    ...
    cd_typo = DB.Column(DB.ForeignKey('ref_habitat.typoref.cd_typo', onupdate='CASCADE'), nullable=False)
    ...
    typoref = DB.relationship('TyporefSHS', primaryjoin='HabrefSHS.cd_typo == TyporefSHS.cd_typo', backref='habrefs')
...
class TTransect(DB.Model):
    ...
    def points_to_linestring(self):
        point1 = ( DB.session.scalar(geo_funcs.ST_X(self.geom_start)), DB.session.scalar(geo_funcs.ST_Y(self.geom_start)) )
        point2 = ( DB.session.scalar(geo_funcs.ST_X(self.geom_end)), DB.session.scalar(geo_funcs.ST_Y(self.geom_end)) )
...
class TPlot(DB.Model):
    ...
    id_transect = DB.Column(DB.ForeignKey('pr_monitoring_habitat_station.t_transects.id_transect', ondelete='CASCADE', onupdate='CASCADE'), nullable=False)
...
class TRelevePlot(DB.Model):
    ...
    id_plot = DB.Column(DB.ForeignKey('pr_monitoring_habitat_station.t_plots.id_plot', ondelete='CASCADE', onupdate='CASCADE'), nullable=False)
    id_base_visit = DB.Column(DB.ForeignKey('gn_monitoring.t_base_visits.id_base_visit', ondelete='CASCADE', onupdate='CASCADE'), nullable=False)
...
class CorHabTaxon(DB.Model):
    ...
    habref = DB.relationship('HabrefSHS', primaryjoin='CorHabTaxon.id_habitat == HabrefSHS.cd_hab', backref='cor_hab_taxons')
...
class CorRelevePlotStrat(DB.Model):
    ...
    id_releve_plot = DB.Column(DB.ForeignKey('pr_monitoring_habitat_station.t_releve_plots.id_releve_plot', onupdate='CASCADE'), nullable=False)
    id_nomenclature_strate = DB.Column(DB.ForeignKey('ref_nomenclatures.t_nomenclatures.id_nomenclature', onupdate='CASCADE'), nullable=False)
...
class CorRelevePlotTaxon(DB.Model):
    ...
    id_cor_releve_plot_taxon = DB.Column(DB.Integer, primary_key=True, server_default=DB.FetchedValue())
    id_releve_plot = DB.Column(DB.ForeignKey('pr_monitoring_habitat_station.t_releve_plots.id_releve_plot', ondelete='CASCADE', onupdate='CASCADE'), nullable=False)
    id_cor_hab_taxon = DB.Column(DB.ForeignKey('pr_monitoring_habitat_station.cor_hab_taxon.id_cor_hab_taxon', onupdate='CASCADE'), nullable=False)
    cover_pourcentage = DB.Column(DB.Integer)
...
class CorTransectVisitPerturbation(DB.Model):
    ...
    id_cor_transect_visit_perturb = DB.Column(DB.Integer, primary_key=True, server_default=DB.FetchedValue())
    id_base_visit = DB.Column(DB.ForeignKey('gn_monitoring.t_base_visits.id_base_visit', ondelete='CASCADE', onupdate='CASCADE'), nullable=False)
